...
test("the center opens as an accessible right-side drawer with a unified first-entry onboarding", () => {
  ...
  assert.match(introSource, /Build momentum, one step at a time/u);
  assert.match(introSource, /const INTRO_POINTS = \[/u);
  assert.equal((introSource.match(/"Choose one clear goal\."|"Add one small next action\."|"Give it a date and priority\."|"Check progress as you go\."/g) || []).length, 4);
  assert.match(introSource, /point\.slice\(0, typedCharacters\)/u);
  assert.match(introSource, /className="goal-todo-intro-points"/u);
  assert.match(introSource, /goal-todo-intro-point/u);
  assert.match(introSource, /goal-todo-intro-start\$\{typingComplete \? " is-ready" : ""\}/u);
  assert.doesNotMatch(introSource, /Navigation|RotateCcw|Pause|Play|goal-todo-intro-(story|card|steps|play|caption)/u);
  ...
  assert.match(introStyles, /\.goal-todo-intro-points\s*\{[\s\S]*?text-align:\s*left;/u);
