...
export default function CometDial({
  ...
  const live = useRef({});
  const reading = useMotionValue(readOnly && !reduce ? min : target.current);
  ...
  const down = e => {
    if (disabled || readOnly || grip.current || e.button > 0) return;
    ...
    try {
      ...
      // Keep keyboard focus if the browser cannot capture this pointer.
    }
  ...
  useEffect(() => {
    if (readOnly && !reduce) launch(target.current, tapBounce, 0);
