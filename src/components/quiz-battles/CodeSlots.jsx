...
// Battle invites use ten uppercase letters and digits 2-9.
const charactersOf = raw => String(raw ?? '').toUpperCase().replace(/[^A-Z2-9]/g, '');
const toSlots = (raw, n) => {
  const d = charactersOf(raw).slice(0, n);
...
export default function CodeSlots({
  ...
  const emitted = useRef(charactersOf(value ?? defaultValue).slice(0, length));
  ...
  const insert = (raw, from = active) => {
    const characters = charactersOf(raw);
  ...
  const onPaste = e => {
    if (busy) return;
    e.preventDefault();
    const pasted = charactersOf(e.clipboardData.getData('text'));
    insert(pasted, pasted.length >= length ? 0 : active);
  };
  const onInput = e => {
    if (busy) return;
    const d = charactersOf(e.target.value);
    if (!d) return;
  ...
  useEffect(() => {
    ...
    const clean = charactersOf(value).slice(0, length);
  ...
  return (
    ...
    >
      ...
      <span id={`${uid}-count`} className="code-slots__sr" aria-live="polite">
        {status === 'success' ? 'Code accepted' : `${view.filter(Boolean).length} of ${length} characters entered`}
      </span>
    </div>
  );
}
