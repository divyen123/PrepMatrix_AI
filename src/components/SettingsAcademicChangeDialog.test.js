...
test("keeps the confirmation portal mounted for smooth close transitions", () => {
  ...
  assert.match(stylesheet, /\.settings-academic-confirm-backdrop\s*\{[^}]*background:\s*rgba\(0, 0, 0, 0\.48\)\s*!important/u);
  assert.match(stylesheet, /backdrop-filter:\s*blur\(18px\)\s*!important/u);
  ...
  assert.match(stylesheet, /linear-gradient\(color-mix\(in srgb, rgb\(var\(--bg-surface-rgb/u);
});
