...
function readings(t, doneOpacity, strikeLag) {
...
export default function SpringCheck({
  ...
  const t = useMotionValue(on && !animateOnMount ? 1 : 0);
  ...
  const write = useCallback((value) => {
    const result = readings(value, doneOpacity, strikeLag);
    if (fillRef.current) fillRef.current.style.transform = result.fill;
    if (boxRef.current) boxRef.current.style.transform = result.box;
    if (tickRef.current) tickRef.current.style.strokeDashoffset = result.tick;
    if (wordRef.current) wordRef.current.style.opacity = result.word;
    if (ruleRef.current) ruleRef.current.style.transform = result.rule;
  }, [doneOpacity, strikeLag]);

  useMotionValueEvent(t, "change", write);
  useLayoutEffect(() => { write(t.get()); }, [t, write]);
  ...
  const result = readings(t.get(), doneOpacity, strikeLag);
  ...
  return (
    <Element
      ...
      className={`spring-check${readOnly ? " is-readonly" : ""}${className ? ` ${className}` : ""}`}
      style={{
        "--sc-ink": color,
        "--sc-fill": fillColor,
        "--sc-check": checkColor,
      ...
      onPointerLeave={readOnly ? undefined : handlePointerCancel}
      onClick={readOnly ? undefined : toggle}
    >
      <span className="spring-check__press">
        <span ref={boxRef} className="spring-check__box" style={{ transform: result.box }}>
          <span className="spring-check__ring" aria-hidden="true" />
          <span ref={fillRef} className="spring-check__fill" style={{ transform: result.fill }} />
          <svg className="spring-check__tick" viewBox="0 0 24 24" aria-hidden="true">
            <path ref={tickRef} d={TICK_PATH} pathLength={1} strokeDasharray={1} style={{ strokeDashoffset: result.tick }} />
          </svg>
        </span>
      </span>
      <span className="spring-check__label">
        <span ref={wordRef} className="spring-check__word" style={{ opacity: result.word }}>{label}</span>
        {strike !== "none" && <span ref={ruleRef} className="spring-check__rule" aria-hidden="true" style={{ transform: result.rule }} />}
      </span>
    </Element>
  );
}
