    ...
    async review(input) {
      ...
      try {
        response = await fetchImpl('https://api.groq.com/openai/v1/chat/completions', {
          ...
          body: JSON.stringify({
            model, temperature: 0.2, max_completion_tokens: 2200,
            response_format: { type: 'json_schema', json_schema: { name: 'code_review', strict: true, schema: CODE_REVIEW_SCHEMA } },
            messages: [{ role: 'system', content: CODE_REVIEW_SYSTEM_PROMPT }, { role: 'user', content: JSON.stringify(input) }],
          }),
        });
      ...
      if (payload?.choices?.[0]?.finish_reason !== 'stop') throw invalid();
      let value;
      try { value = JSON.parse(payload.choices[0].message.content); } catch { throw invalid(); }
