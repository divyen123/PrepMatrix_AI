...
**Optional AI Code Assistant:** Set `CODEMATRIX_AI_API_KEY` to a dedicated Groq API key in the backend environment and restart the server. It never falls back to the main Groq key and must not use a `VITE_` prefix. `CODEMATRIX_AI_MODEL` defaults to `openai/gpt-oss-20b`; overrides must support strict JSON schema responses. Running code remains free of AI credit charges.

After a code error, students can choose **Review this error** to request focused hints, pitfalls, a source location, and a test to try. The assistant is instructed to leave the solution to the student; responses are validated as bounded prose without code blocks. The cost is shown before the request: `AI_CREDIT_COST_CODE_REVIEW` defaults to **1 credit** from that user's existing monthly allowance, shared across their academic profiles. There is no separate global app-credit pool for this feature; provider billing and rate limits still apply to the dedicated key. Identical saved reviews replay without another charge while retained in the usage ledger. Failed generation releases the reserved credit; uncertain network results can be retrieved by retrying the same review. Editing code marks an earlier review as outdated until the student runs the updated code.

Incomplete or invalid hints receive one automatic corrective attempt within the same 40-second deadline and credit reservation. Each attempt is checked by the prose validator; if neither succeeds, the credit is refunded. GPT-OSS uses low reasoning effort to leave room for the final hint within the output budget.
